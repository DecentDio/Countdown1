...
const bookList = [
  ...
  //one
  const one = (titleN, authorN) => {
    let ans = {title: titleN, author: authorN};
    bookList.push(ans);
  }
  //two

  const two = (title) => {
    let numb = bookList.indexOf(title);
    bookList.splice(numb, 1);
  }

  //three
  const three = () => {
    bookList.forEach((Object) => console.log(Object.author));
  }
...
const four = (inte) => inte > 0 ? "Positive" : "Negative";
